...
int main(){
    ...
    maxinteval.push_back({{0, initial_arr[0].end - 1}, 0LL});
    ...
    for(long long k = 0 ; k < n - 1; k++)
    {
        ...
        maxinteval.push_back({{initial_arr[k].end, initial_arr[k+1].end - 1}, x});
        /*cout<<"\n";
    ...
    long long x = max(initial_arr[n-1].value + bi_search_max(maxinteval, initial_arr[n-1].start), maxinteval[n-2].second);
